...
void BuildResultAreaSnapshot(SlaveData *pDat, int id, char *buf, size_t bufSize)
{
	if (buf == NULL || bufSize == 0)return;
	buf[0] = 0;
	if (pDat == NULL || pDat->map_result == NULL)
	{
		sprintf_s(buf, bufSize, "[DLL_FLOW]ResultArea,ws=%d,pc=%d,map_result=NULL", GetWsNoByPcId(id), id);
		return;
	}

	int mark = pDat->map_result->tab_registers[REG_IDX_MARK];
	int status = pDat->map_result->tab_registers[REG_IDX_STATUS];
	int param = pDat->map_result->tab_registers[REG_IDX_PARAM];
	int num = pDat->map_result->tab_registers[REG_IDX_DAT_LEN];
	int showCnt = num > 0 ? num : 8;
	if (showCnt > INF_CNT)showCnt = INF_CNT;

	size_t offset = 0;
	int written = sprintf_s(buf, bufSize, "[DLL_FLOW]ResultArea,ws=%d,pc=%d,mark=%d,sta=%d,param=%d,num=%d,res=[",
		GetWsNoByPcId(id), id, mark, status, param, num);
	if (written < 0)return;
	offset = (size_t)written;

	for (int i = 0; i < showCnt && offset + 16 < bufSize; i++)
	{
		int val = pDat->map_result->tab_registers[REG_IDX_DAT + i * 2 + 1] << 16 | pDat->map_result->tab_registers[REG_IDX_DAT + i * 2];
		written = sprintf_s(buf + offset, bufSize - offset, i == 0 ? "%d" : ",%d", val);
		if (written < 0)break;
		offset += (size_t)written;
	}
	...
	if (offset + 2 < bufSize)
	{
		sprintf_s(buf + offset, bufSize - offset, "]");
	}
}

void BuildDeviceResultAreaSnapshot(char *buf, size_t bufSize)
{
	if (buf == NULL || bufSize == 0)return;
	buf[0] = 0;
	if (Dev.mb_map_result == NULL)
	{
		sprintf_s(buf, bufSize, "[DLL_TX]ResultArea,ws=%d,pc=%d,map_result=NULL", GetWsNoByPcId(Dev.slaveid), Dev.slaveid);
		return;
	}

	int mark = Dev.mb_map_result->tab_registers[REG_IDX_MARK];
	int status = Dev.mb_map_result->tab_registers[REG_IDX_STATUS];
	int param = Dev.mb_map_result->tab_registers[REG_IDX_PARAM];
	int num = Dev.mb_map_result->tab_registers[REG_IDX_DAT_LEN];
	int showCnt = num > 0 ? num : 8;
	if (showCnt > INF_CNT)showCnt = INF_CNT;

	size_t offset = 0;
	int written = sprintf_s(buf, bufSize, "[DLL_TX]ResultArea,ws=%d,pc=%d,mark=%d,sta=%d,param=%d,num=%d,res=[",
		GetWsNoByPcId(Dev.slaveid), Dev.slaveid, mark, status, param, num);
	if (written < 0)return;
	offset = (size_t)written;

	for (int i = 0; i < showCnt && offset + 16 < bufSize; i++)
	{
		int val = Dev.mb_map_result->tab_registers[REG_IDX_DAT + i * 2 + 1] << 16 | Dev.mb_map_result->tab_registers[REG_IDX_DAT + i * 2];
		written = sprintf_s(buf + offset, bufSize - offset, i == 0 ? "%d" : ",%d", val);
		if (written < 0)break;
		offset += (size_t)written;
	}
...
void BuildInfAreaSnapshot(SlaveData *pDat, int id, char *buf, size_t bufSize)
{
	if (buf == NULL || bufSize == 0)return;
	buf[0] = 0;
	if (pDat == NULL || pDat->map_inf == NULL)
	{
		sprintf_s(buf, bufSize, "[DLL_FLOW]InfArea,ws=%d,pc=%d,map_inf=NULL", GetWsNoByPcId(id), id);
		return;
	}

	int mark = pDat->map_inf->tab_registers[REG_IDX_MARK];
	int status = pDat->map_inf->tab_registers[REG_IDX_STATUS];
	int param = pDat->map_inf->tab_registers[REG_IDX_PARAM];
	int num = pDat->map_inf->tab_registers[REG_IDX_DAT_LEN];
	char barcode[65] = { 0 };
	char* pch = (char*)&pDat->map_inf->tab_registers[REG_IDX_DAT];
	strncpy_s(barcode, sizeof(barcode), pch, _TRUNCATE);

	sprintf_s(buf, bufSize, "[DLL_FLOW]InfArea,ws=%d,pc=%d,mark=%d,sta=%d,param=%d,num=%d,barcode=%s",
		GetWsNoByPcId(id), id, mark, status, param, num, barcode[0] == 0 ? "EMPTY" : barcode);
}

void BuildDeviceInfAreaSnapshot(char *buf, size_t bufSize)
{
	if (buf == NULL || bufSize == 0)return;
	buf[0] = 0;
	if (Dev.mb_map_inf == NULL)
	{
		sprintf_s(buf, bufSize, "[DLL_TX]InfArea,ws=%d,pc=%d,map_inf=NULL", GetWsNoByPcId(Dev.slaveid), Dev.slaveid);
		return;
	}

	int mark = Dev.mb_map_inf->tab_registers[REG_IDX_MARK];
	int status = Dev.mb_map_inf->tab_registers[REG_IDX_STATUS];
	int param = Dev.mb_map_inf->tab_registers[REG_IDX_PARAM];
	int num = Dev.mb_map_inf->tab_registers[REG_IDX_DAT_LEN];
	char barcode[65] = { 0 };
...

{
	...
	DebugStr5("[DLL_FLOW]StartTestFlow Enter,id=%d,ws=%d,req_sta=%d,result_mark=%d,inf_mark=%d\n",
		id,
		GetWsNoByPcId(id),
		status,
			...
			*status = *status & 0x0FFF;
...

			{
...

				{
					...
					DebugStr5("[DLL_FLOW]WaitTestResult Return,id=%d,ws=%d,result_mark=%d,status=%d,num=%d\n",
						id,
						GetWsNoByPcId(id),
						curMark,
						*status,
