...
namespace MotionCtrl
{
    public static class Utility
    {
        ...
        public static void CatchDiskAndDeleteFile(string HardDiskName, string DeletePath, int DeleteDay,bool isfile=true, int freeSpace = 10)
        {
            ...
            if (System.IO.Directory.Exists(DeletePath))
            {
                if (FreeSpace < freeSpace)
                {
                    ...
                    {
                        foreach (DirectoryInfo DeInfo in DyInfo.GetDirectories())
                        {
                            if (DeInfo.CreationTime < DateTime.Now.AddDays(DeleteDay))
                            {
                                Directory.Delete(DeInfo.FullName,true);
                            }
                        }
                    }
        ...
        public static DataTable ReadFromXml(string filename)
        {
            ...
            {
                MessageBox.Show(ex.Message, VAR.IsChinese?"错误":"ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public static bool DataGridViewToCSV(string filename,DataGridView dataGridView)
        {
            lock (objCSV)
            {
                ...
                try
                {
                    StreamWriter sw = new StreamWriter(filename, true, System.Text.Encoding.GetEncoding("UTF-8"));
                    string strLine = "";
                    //表头
                    ...
                    sw.Close();
                    sw.Dispose();
                    //MessageBox.Show("数据被导出到：" + saveFileDialog.FileName.ToString(), "导出完毕", MessageBoxButtons.OK,
        ...
        #endregion
        #region 打开PDF并跳转到设定页面

        public static bool OpenPDF(string filename,int OpenPageNum=1)
        {
           // int pageNum = 2;
            //string _EMDGuidecardsPdfPath = "C:\\Users\\George Lin\\Desktop\\c#中await_async闲说.pdf";
            if (System.IO.File.Exists(filename))
            {
                string adobeProcessName = "AcroRd32";
                System.Diagnostics.Process[] adobeProcessArr = System.Diagnostics.Process.GetProcessesByName(adobeProcessName);
                if (adobeProcessArr.Length > 0)
                {
                    try
                    {
                        adobeProcessArr[0].Kill();
                    }
                    catch
                    {
                    }
                ...
                //pageNum = 10;// GetPageNum();
                System.Diagnostics.Process adobeProcess = new System.Diagnostics.Process();
                adobeProcess.StartInfo.FileName = adobeProcessName + ".exe";
                adobeProcess.StartInfo.Arguments = "/A \"page=" + OpenPageNum + "=OpenActions\" \"" + filename + "\"";
                try
                {
                    adobeProcess.Start();
                }
                ...
                {
                    MessageBox.Show(VAR.IsChinese?"请确认是否已安装PDF程序!": "Please confirm whether the PDF program is installed!\r\n请确认是否已安装PDF程序!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }
            }
            else
            {
                MessageBox.Show(VAR.IsChinese?"找不到以下路径文件:\r\n" + filename: "The following path files were not found:\r\n" + filename+ "\r\n找不到以下路径文件:\r\n" + filename, "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            ...
            int row_y, bool brow_inc = false, bool brolback = false, bool bshort_step_mode = false)
        {
            ...
            if (col_x <= 0 && row_y <= 0)
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?"阵列数据异常!\r\n行数或列数小于1!": "Array data is abnormal!\r\nNumber of rows or columns is less than 1!(阵列数据异常!\r\n行数或列数小于1!)", DReport.EmErrCode.GetParamError);
                return null;
            }
