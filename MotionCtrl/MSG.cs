...
namespace MotionCtrl
{
    ...
    public class Msg
    {
        public static SecsManager secsManager = new SecsManager();
        public static List<Alarm> AlarmInfo = new List<Alarm>();
        ...
        public static FileStream Log, Log_Alm, Log_Info;
        ...
        public static void SaveMsg(ref FileStream log, string MsgStr)
        {
            byte[] byData;
            // if (log == null) log = Log;
            if (log == null || 0 != string.Compare(str_date, DateTime.Now.ToString("yyyy-MM-dd")))
            {
                ...
                try
                {
                    ...
                    if (log == Log) str += DateTime.Now.ToString("yyyy-MM-dd") + ".log";
                    else if (log == Log_Alm) str += DateTime.Now.ToString("yyyy-MM-dd") + "_alm.log";
                    else if (log == Log_Info) str += DateTime.Now.ToString("yyyy-MM-dd") + "_info.log";
                    if (!File.Exists(str))
                    {
        ...
        public void showmsg(TextBox tb)
        {
            ...
            while (list_msgdat.Count > 0)
            {
                ...
                {
                    SaveMsg(ref Log, msg.ToString());
                    if (((int)msg.msg_type & ((int)MsgCfg) >> 8) == (int)EM_MSGTYPE.ERR) SaveMsg(ref Log_Alm, msg.ToString());
        ...
        public void AddMsg(EM_MSGTYPE MsgType, string MsgStr, DReport.EmErrCode emerr = DReport.EmErrCode.Null, int relevanthw = 0, ERR_ALM.EmErrItem emErr = ERR_ALM.EmErrItem.Null, ShowErrMsg ErrCode = ShowErrMsg.None)
        {
            ...
            try
            {
                ...
                lock (LockObj)
                {
                    if (emErr != ERR_ALM.EmErrItem.Null && MsgType == EM_MSGTYPE.ERR)
                    {
                        VAR.SysErrAlm.ErrStr = MsgStr;
                        VAR.SysErrAlm.ErrItem = emErr;
                    }
    ...
    public class SYS_INF
    {
        ...
        private void timer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            //beeper
            if (beeptimeupdate && !PT_SET.bContinuousAlarm)
            {
                ...
                if (beeptimer >= Math.Min(beeptime, maxbeeptime))
                {
                    ...
                    beeptimeupdate = false;
                    beeptime = 0;
                    //beeptimer++;
                }
            }
            else if (beeptimeupdate && PT_SET.bContinuousAlarm)
            {
                if (VAR.gsys_set.status != EM_SYS_STA.RUN) out_alm_beer.SetOff();
            }

        }

        delegate void DisplayCallback(object displayer);
        private bool IsHaveForm(string formName)
        {
