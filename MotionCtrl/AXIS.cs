...
namespace MotionCtrl
{
    ...
    public class AXIS
    {
        ...
        public AXIS(ushort ax_id, CARD card, string disc,string english_disc="" ,MT_TYPE mt_type = MT_TYPE.SEVER, ENC_TYPE encode = ENC_TYPE.YES, GPIO.IO_STA logic_svr_on = GPIO.IO_STA.OUT_ON, int cmd_k = 1, int enc_k = 1)
        {
            this.card = card;
            if (card != null)
            ...
            {
                pul_per_mm = 0;
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR,VAR.IsChinese?String.Format("{0} io_num {1} 超范围[{2},{3})", disc, num, 0, card.ax_num): String.Format("{0} io_num {1} Over range[{2},{3})   ({4} io_num {1} 超范围[{2},{3}))", english_disc, num, 0, card.ax_num,disc));
            ...
            {
                pul_per_mm = 0;
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? String.Format("{0} 设置参数异常！", disc): String.Format("{0} Setting parameter exception! ({1} 设置参数异常！)", english_disc, disc));
            }
        ...
        public EM_RES SaveCfgToInf(string filename = "")
        {
            ...
            inf.WriteString(Section, "DISC", str_disc,ref ischange, true, filename);
            inf.WriteString(Section, "MT_TYPE", Enum.GetName(typeof(MT_TYPE), mt_type), ref ischange, true, filename);
            inf.WriteString(Section, "ENCODE", Enum.GetName(typeof(ENC_TYPE), encode), ref ischange, true, filename);
            inf.WriteInteger(Section, "LOGIC_SVR_ON", (int)logic_svr_on, ref ischange, true, filename);

            inf.WriteInteger(Section, "HOME_DIR", (int)home_dir, ref ischange, true, filename);
            //inf.WriteInteger(Section, "HOME_MODE", home_mode);
            inf.WriteDouble(Section, "HOME_OFFSET", home_offset, ref ischange, true, filename);
            inf.WriteDouble(Section, "HOME_SPD", home_spd, ref ischange, true, filename);

            inf.WriteDouble(Section, "WORK_SPD", spd_work, ref ischange, true, filename);
            inf.WriteDouble(Section, "START_SPD", spd_start, ref ischange, true, filename);
            inf.WriteDouble(Section, "STOP_SPD", spd_stop, ref ischange, true, filename);
            inf.WriteDouble(Section, "MAN_SPD_H", spd_manual_high, ref ischange, true, filename);
            inf.WriteDouble(Section, "MAN_SPD_L", spd_manual_low, ref ischange, true, filename);

            inf.WriteDouble(Section, "PUL_PER_MM", pul_per_mm, ref ischange, true, filename);
            inf.WriteDouble(Section, "SLP", slp, ref ischange, true, filename);
            inf.WriteDouble(Section, "SLN", sln, ref ischange, true, filename);
            inf.WriteDouble(Section, "TACC", tacc, ref ischange, true, filename);
            inf.WriteDouble(Section, "TDEC", tdec, ref ischange, true, filename);
            inf.WriteDouble(Section, "TS", ts, ref ischange, true, filename);

            //inf.WriteDouble(Section, "MAX_ACC", max_acc);
            //inf.WriteDouble(Section, "MAX_SPD", max_spd);

            inf.WriteInteger(Section, "AX_TYPE", ax_type, ref ischange, true, filename);
            inf.WriteInteger(Section, "PULS_MODE", puls_mode, ref ischange, true, filename);

            inf.WriteInteger(Section, "ELP", elp_num, ref ischange, true, filename);
            inf.WriteBool(Section, "ELP_ACTIVE_H", elp_active_h, ref ischange, true, filename);
            inf.WriteInteger(Section, "ELN", eln_num, ref ischange, true, filename);
            inf.WriteBool(Section, "ELN_ACTIVE_H", eln_active_h, ref ischange, true, filename);
            inf.WriteInteger(Section, "ORG", org_num, ref ischange, true, filename);
            inf.WriteBool(Section, "ORG_ACTIVE_H", org_active_h, ref ischange, true, filename);
            inf.WriteInteger(Section, "ALM", alm_num, ref ischange,true,filename);
            inf.WriteBool(Section, "ALM_ACTIVE_H", alm_active_h, ref ischange, true, filename);
            ...
            if (ischange)
            {
                res = SYS_PUD.CopyFile2(Path.GetFullPath("..") + "\\syscfg\\backup");
                if (res != EM_RES.OK) return res;
                res = SYS_PUD.FileWriteLine("axiscfg.ini",backup,Path.GetFullPath("..") + "\\syscfg\\backup");
                if (res != EM_RES.OK) return res;
            }
            return EM_RES.OK;
        }
        ...
        public EM_RES SetELMode(EL_EN el_en, EL_LOGIC el_logic = EL_LOGIC.NP_L, EL_STOP el_stop = EL_STOP.NP_EMG)
        {
            ...
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?String.Format("{0} 设置限位使能异常，el_en={1}！", disc, (int)el_en): String.Format("{0}  Setting limit enable abnormal,el_en={1}! ({0} 设置限位使能异常，el_en={1}！)",disc, (int)el_en));
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (err != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?String.Format("{0} 设置限位模式出错，{1}！", disc, err): String.Format("{0} Error setting limit mode,{1}！ ({0} 设置限位模式出错，{1}！)", disc, err));
            ...
            if (card.brand == CARD.BRAND.ZMOTION)
            {
                ...
                if (err != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?String.Format("{0} 设置限位IO编号出错，{1}！", disc, err): String.Format("{0}  Error setting limit IO number，{1}！({0} 设置限位IO编号出错，{1}！)", disc, err));
                    return res = EM_RES.ERR;
                }
        ...
        public EM_RES MtParamCfg(double pul_per_mm, double spd_work, double spd_manual_high, double spd_manual_low, double tacc, double tdec, double spd_start = 0, double spd_stop = 0)
        {
            ...
            if (tacc < min_tacc) { VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 加速时间{1:F3} 过小（<{2:F3}）", disc, tacc, min_tacc): string.Format("{0} tacc{1:F3} too short （<{2:F3})({0} 加速时间{1:F3} 过小（<{2:F3}）)", disc, tacc, min_tacc)); res = EM_RES.PARA_ERR; }
            if (tdec < min_tacc) { VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("{0} 减速时间{1:F3} 过小 (<{2:F3}）", disc, tacc, min_tacc): string.Format("{0} tdec{1:F3} too short (<{2:F3}）({0} 减速时间{1:F3} 过小 (<{2:F3}）)", disc, tacc, min_tacc)); res = EM_RES.PARA_ERR; }
        ...
        public EM_RES Init()
        {
            ...
            if (card.brand == CARD.BRAND.ZMOTION)
            {
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese?String.Format("{0} 获取负限位（IN={1}）出错，{2}！", disc, eln_num, err): String.Format("ERROR:{0} Gets the negative limit (IN={1}),{2}! ({0} 获取负限位（IN={1}）出错，{2}！)", disc, eln_num, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                    return  EM_RES.ERR;
                }
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese ? String.Format("{0} 获取正限位（IN={1}）出错，{2}！", disc, elp_num, err): String.Format("{0} ERROR:Gets the positive limit (IN={1}),{2}! ({0} 获取正限位（IN={1}）出错，{2}！)",disc, elp_num, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                    return  EM_RES.ERR;
                }
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese ? String.Format("{0} 获取原点（IN={1}）出错，{2}！", disc, org_num, err): String.Format("{0} Acquire to the origin （IN={1}), {2}! ({0} 获取原点（IN={1}）出错，{2}！）",  disc, org_num, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                    return  EM_RES.ERR;
                }
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese ? String.Format("{0} 获取轴报警(IN={1}）出错，{2}！", disc, alm_num, err): String.Format("{0} Acquire axis ALM(IN={1}), {2}! ({0} 获取轴报警(IN={1}）出错，{2}！)", disc, alm_num, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                    return  EM_RES.ERR;
                }
        ...
        public EM_RES ChkParam()
        {
            ...
            if (m_id < 0)
            {
                ErrMes = VAR.IsChinese?String.Format("{0} ID({1})异常!", disc, m_id): String.Format("{0} ID({1}) ERR! ({0} ID({1})异常!)",disc, m_id);
                if (bShowErrMsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                bShowErrMsg = false;
                res = EM_RES.MOVE_PARA_ERR;
                return res;
            }

            if (card == null || !card.isReady)
            {
                ErrMes = VAR.IsChinese ? String.Format("{0} 对应板卡{1}未初始化!", disc, card.disc): String.Format("{0} {1} is not initialized  ({0} 对应板卡{1}未初始化! )",  disc, card.disc);
                if (bShowErrMsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.InitFailed, (int)DReport.EmHareware.Axis + m_id);
                bShowErrMsg = false;
                res = EM_RES.MOVE_PARA_ERR;
                return res;
            }
            ...
            {
                ErrMes = VAR.IsChinese ? String.Format("{0} 轴参数未初始化!", disc): String.Format("{0} Axis parameters are not initialized! ({0} 轴参数未初始化!)",disc);
                if (bShowErrMsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.InitFailed, (int)DReport.EmHareware.Axis + m_id);
                bShowErrMsg = false;
                res = EM_RES.MOVE_PARA_ERR;
                return res;
        ...
        public string disc
        {
            get
            {
                return string.Format("{0}({1}/{2})", VAR.IsChinese?str_disc.Replace("\0", ""): english_disc.Replace("\0", ""), num, card.id);
            }
        }
        #endregion
        public class AXSTA
        ...
        public double spd_cur
        {
            get
            {
                ...
                if (card.brand == CARD.BRAND.LEADSHINE)
                {
                    ...
                    if (err != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 获取速度失败，Err：{1}", disc, err): string.Format("ERROR:{0} Acquire speed,{1}({0} 获取速度失败，Err：{1})", disc, err), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                        res = EM_RES.ERR;
                        return mspd_cur;
                    }
                }
        ...
        public int cmd_pos
        {
            ...
            set
            {
                ...
                if (err != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置CMD坐标出错，Err:0x{1:X8}", disc, err): string.Format("{0} ERROR: Set CMD coordinates, 0x{1:X8}({0} 设置CMD坐标出错，Err:0x{1:X8})",  disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.ERR;
                    return;
                }
        ...
        public int enc_pos
        {
            ...
            set
            {
                ...
                if (err != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置编码位置出错，0x{1:X8}", disc, err): string.Format("ERROR:{0} Set Encoding location,0x{1:X8} ({0} 设置编码位置出错，0x{1:X8})",  disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.ERR;
                    return;
                }
                //update
        ...
        public EM_RES EnSlm(bool ben)
        {
            ...
            if (err != 0)
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置软限位出错，Err:{1}", disc, err): string.Format("{0} Set software limit,{1} ({0} 设置软限位出错，Err:{1})",  disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.ERR;
                return EM_RES.ERR;
            }
        ...
        #endregion
        #region 设定速度
        public EM_RES ChangSpeed(double spd_chage)
        {
            string ErrMes = string.Empty;
         #if LEADSHINE
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (Math.Abs(spd_chage) > Math.Abs(max_spd))
                {
                    ErrMes = VAR.IsChinese?string.Format("{0} 速度设置 {1:F3} 超范围[0,{2:F3}]！", disc, spd_chage, max_spd): string.Format("{0} Set speed  {1:F3} out of range [0,{2:F3}]！  ({0} 速度设置 {1:F3} 超范围[0,{2:F3}]！)", disc, spd_chage, max_spd);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id,ERR_ALM.EmErrItem.ParmError);
                    res = EM_RES.PARA_ERR;
                    return res;
                }
                ...
                err = LTDMC.dmc_change_speed((ushort)card.card_id, (ushort)num, spd_chage * pul_per_mm, 0);
                if (err == 4)
                {
                    ErrMes = VAR.IsChinese?string.Format("{0} S曲线设置,板卡忙", disc): string.Format("{0} ERROR:Card is busy,cant change S speed  ({0} S曲线设置,板卡忙)", disc);                   
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.BUSY;
                    return res;
                }
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese ? string.Format("{0} S曲线设置出错,ts={1:F3}ms,Err:{2}", disc, ts, err): string.Format("{0} ERROR:Set S speed,ts={1:F3}ms,Err:{2}   ({0} S曲线设置出错,ts={1:F3}ms,Err:{})",  disc, ts, err);                    
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                    res = EM_RES.ERR;
                    return res;
                }
        ...
        public EM_RES SetSpeed(double spd_start, double spd_high, double spd_stop, double Tacc, double Tdec)
        {
            ...
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 未设置速度！", disc): string.Format("{0} Did't set speed!({1} 未设置速度！)",english_disc, disc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                return EM_RES.PARA_ERR;
            }
            ...
            {
                ErrMes = VAR.IsChinese ? string.Format("{0} 未设置最大速度/最大加速度！", disc): string.Format("{0} Did't set max speed or max acc!  ( {0} 未设置最大速度/最大加速度！)", disc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.PARA_ERR;
                return res;
            }
            ...
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 速度设置 [{1:F3},{2:F3}] 超范围[0,{3:F3}]！", disc, spd_start, spd_high, max_spd): string.Format("{0} Speed setting [{1:F3},{2:F3}] is out of range[0,{3:F3}]！  ({0} 速度设置 [{1:F3},{2:F3}] 超范围[0,{3:F3}]！)",  disc, spd_start, spd_high, max_spd);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.PARA_ERR;
                return res;
            }
            ...
            {
                ErrMes = VAR.IsChinese ? string.Format("{0} 脉冲速度 {1} < 10，异常", disc, spd_high * pul_per_mm): string.Format("{0}ERROR: spe_pul {1} <10   ({0} 脉冲速度 {1} < 10，异常)", disc, spd_high * pul_per_mm);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.WAR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                //res = EM_RES.PARA_ERR;
                //return res;
            }
            ...
            if (max_acc >= 20000) min_tacc = 0.08;
            if (Math.Abs(Tacc) < Math.Abs(min_tacc) || Math.Abs(Tdec) < Math.Abs(min_tacc))
            {
                ErrMes = VAR.IsChinese ? string.Format("{0} 加/减速时间 Tacc/Tdec{1:F3}/{2:F3} 过小 (<{3:F3})！", disc, Tacc, Tdec, min_tacc): string.Format("{0}  Tacc/Tdec{1:F3}/{2:F3} too short  (<{3:F3})！  ({0} 加/减速时间 Tacc/Tdec{1:F3}/{2:F3} 过小 (<{3:F3})！)",  disc, Tacc, Tdec, min_tacc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR,ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.PARA_ERR;
                return res;
            }
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (err == 4)
                {
                    ErrMes = VAR.IsChinese?string.Format("{0} 速度设置,板卡忙", disc): string.Format("{0} Card is busy,can't set speed  ({0} 速度设置,板卡忙)", disc);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.BUSY;
                    return res;
                }
            ...
            if (err != 0)
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 速度设置出错,Err:{1}", disc, err): string.Format("{0} ERROR:Set speed {1}  ({0} 速度设置出错,Err:{1})", disc, err);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.ERR;
                return res;
            }
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (err == 4)
                {
                    ErrMes = VAR.IsChinese?string.Format("{0} S曲线设置,板卡忙", disc): string.Format("{0}  Card is busy,can't set s profile  ({0} S曲线设置,板卡忙)", disc);                   
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.BUSY;
                    return res;
                }
            ...
            if (err != 0)
            {
                ErrMes = VAR.IsChinese?string.Format("{0} S曲线设置出错,ts={1:F3}ms,Err:{2}", disc, ts, err): string.Format("{0} ERROE:Set s profile ,ts={1:F3}ms,Err:{2} ({0} S曲线设置出错,ts={1:F3}ms,Err:{2})",  disc, ts, err);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                res = EM_RES.ERR;
                return res;
            }
        ...
        public AX_STA status
        {
            get
            {
                ...
                if ((iostatus & (uint)MTIO.EMG) == (uint)MTIO.EMG)
                    ...
                    sta = AX_STA.ALM;
                else if ((iostatus & (uint)MTIO.SVON) != (uint)MTIO.SVON && mt_type != MT_TYPE.STEP)
                    sta = AX_STA.DIS;
                ...
                if (sta == AX_STA.EMG|| sta == AX_STA.ALM || sta == AX_STA.ELP|| sta == AX_STA.ELN)
                {
                    home_status = HOME_STA.UNKOWN;
                }
                res = EM_RES.OK;
        ...
        public string str_status
        {
            get
            {
                ...
                {
                    case AX_STA.ALM:
                        return VAR.IsChinese?"报警":"ALM";
                    case AX_STA.EMG:
                        return VAR.IsChinese ? "急停":"EMG";
                    case AX_STA.ELP:
                        return VAR.IsChinese ? "正限位": "ELP";
                    case AX_STA.ELN:
                        return VAR.IsChinese ? "负限位": "ELN";
                    case AX_STA.PTP:
                        return VAR.IsChinese ? "运行" : "PTP";
                    case AX_STA.SLP:
                        return VAR.IsChinese ? "正软限位" : "SLP";
                    case AX_STA.SLN:
                        return VAR.IsChinese ? "负软限位" : "SLN";
                    case AX_STA.DIS:
                        return VAR.IsChinese ? "失电" : "DIS";
                    case AX_STA.HOMEING:
                        return VAR.IsChinese ? "回零中" : "HOMEING";
                    case AX_STA.NRDY:
                        return VAR.IsChinese ? "未就绪" : "NRDY";
                    case AX_STA.READY:
                        return VAR.IsChinese ? "就绪" : "READY";
                    case AX_STA.DISCONNECT:
                        return VAR.IsChinese ? "掉线" : "DISCONNECT";
                    default:
                        break;
                }
                return VAR.IsChinese ? "未知" : "Unknown";
            }
        }
        ...
        public bool SVRON
        {
            ...
            set
            {
                ...
                if (err != 0)
                {                   
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置SVRON出错，0x{1:X8}", disc, err): string.Format("{0}  ERROR:Set SVRON，0x{1:X8} ,({0} 设置SVRON出错，0x{1:X8})",  disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.ERR;
                    return;
                }
                res = EM_RES.OK;
        ...
        public EM_RES SetHandwheel(ushort channel = 0)
        {
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (err != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 手轮设置出错，0x{1:X8}", disc, err): string.Format("{0}  ERROR:Set handwheel_channel，0x{1:X8} ({0} 手轮设置出错，0x{1:X8})", disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    res = EM_RES.ERR;
                    return res;
                }
            }
        ...
        public bool ERC
        {
            ...
            set
            {
                ...
                if (card.brand == CARD.BRAND.LEADSHINE)
                {
                    ...
                    if (err != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置ERC出错，0x{1:X8}", disc, err): string.Format("{0} ERROR:Set ERC,0x{1:X8}  ({0} 设置ERC出错，0x{1:X8})",  disc, err), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                        res = EM_RES.ERR;
                    }
                    else res = EM_RES.OK;
                }
        ...
        public EM_RES SetHcmp(double[] pos, ushort io_num, ushort mode = 4,bool IfClear=true)
        {
            string ErrMes = string.Empty;
            //check
            res = ChkParam();
            ...
            //config,L=active,T=500us
            res = (EM_RES)LTDMC.dmc_hcmp_set_config((ushort)card.card_id, cmp, (ushort)num, 1, 0, 1000);
            if (res != EM_RES.OK)
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 触发设置出错,{1}", disc, res): string.Format("{0} ERROR: Set hcmp config{1}  ({0} 触发设置出错,{1})", disc, res);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                return res;
            }
            ...
            if (res != EM_RES.OK)
            {
               ErrMes = VAR.IsChinese ? string.Format("{0} 触发模式设置出错 Err,{1}", disc, res): string.Format("{0} ERROR: Set hcmp config{1}  ({0} 触发模式设置出错 Err,{1})", disc, res);
               VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                return res;
            }
            Thread.Sleep(10);
            ...
            if (IfClear)
            {
                ...
                if (res != EM_RES.OK)
                {
                    ErrMes = VAR.IsChinese ? string.Format("{0} 清除触发点出错 Err,{1}", disc, res): string.Format("{0} ERROR:Clear points {1}  ({0} 清除触发点出错 Err,{1})", disc, res);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes,DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                    return res;
                }
            }
            ...
            if (mode != 0)
            {
                ...
                for (int n = 0; n < pos.Length; n++)
                {
                    ...
                    {
                        ErrMes = VAR.IsChinese?string.Format("{0} 增加触发点 {1:F3} 超软限位范围[{2:F3},{3:F3}]！", disc, pos[n], sln, slp): string.Format("{0} add points {1:F3} over range [{2:F3},{3:F3}]！  ({0} 增加触发点 {1:F3} 超软限位范围[{2:F3},{3:F3}]！)", disc, pos[n], sln, slp);
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                        res = EM_RES.PARA_ERR;
                        return res ;
                    }
                    ...
                    if (res != EM_RES.OK)
                    {
                        ErrMes = VAR.IsChinese ? string.Format("{0} 增加触发点出错,{1}", disc, res): string.Format("{0} ERROR:Add points,{1}  ({0} 增加触发点出错,{1})", disc, res);
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                        return res;
                    }
                }
        ...
        public EM_RES Stop(bool bemg = false)
        {
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (bemg || err != 0)
                {
                    ...
                    if (err != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 停止出错，{1}", disc, res): string.Format("{0} ERROR:Stop,{1}  ({0} 停止出错，{1})", disc, res), DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                        res = EM_RES.ERR;
                    }
                }
            }
        ...
        public EM_RES WaitHomeStop(ref bool bquit, int timeout_ms = 10000)
        {
            ...
            while (true)
            {
                ...
                if (bquit)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 异常停止,{1}！", disc, str_status): string.Format("{0} ERROR:Stop,{1}！  ({0} 异常停止,{1}！)", disc, str_status), DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                    Stop();
                ...
                if (card.brand == CARD.BRAND.LEADSHINE)
                {
                    ...
                    if (1 == LTDMC.dmc_check_done((ushort)card.card_id, (ushort)num))
                    {
                        ...
                        if (1 == LTDMC.dmc_check_done((ushort)card.card_id, (ushort)num))
                        {
                            ...
                            {
                                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 异常停止,{1}！", disc, str_status): string.Format("{0} ERROE:Stop ,{1}！   ({0} 异常停止,{1}！)",  disc, str_status), DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                                Stop();
                                return res = EM_RES.ERR;
                            }
                            break;
                ...
                if (card.brand == CARD.BRAND.ZMOTION)
                {
                    ...
                    if (err == 0 && temp == -1)
                    {
                        if (bquit)
                        {
                            VAR.msg.AddMsg(Msg.EM_MSGTYPE.WAR, VAR.IsChinese?string.Format("{0} 用户取消停止！", disc): string.Format("{0} User cancel stop  ({0} 用户取消停止！)", disc));
                            return res = EM_RES.QUIT;
                        }
                ...
                {
                    Stop();
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 等待停止超时({1}ms)！", disc, timeout): string.Format("{0} Wait to stop timeout ({1}ms)！ ({0} 等待停止超时({1}ms)！)",  disc, timeout), DReport.EmErrCode.Timeout, (int)DReport.EmHareware.Axis + m_id);
        ...
        public EM_RES WaitForStop(ref bool pbquit, int timeout_ms,bool bdoevent = false, bool bshowmsg=true)
        {
            ...
            while (true)
            {
                ...
                if (VAR.gsys_set.bquit) res = EM_RES.QUIT;
                ...
                if (card.brand == CARD.BRAND.LEADSHINE)
                {
                    ...
                    if (1 == LTDMC.dmc_check_done((ushort)card.card_id, (ushort)num))
                    {
                        ...
                        if (1 == LTDMC.dmc_check_done((ushort)card.card_id, (ushort)num))
                        {
                            if (pbquit || VAR.gsys_set.bquit) return res = EM_RES.QUIT;
                            if (status != AX_STA.READY)
                            {
                                ErrMes = VAR.IsChinese?string.Format("{0} 异常停止,{1}！", disc, str_status): string.Format("{0} ERROR:Stop,{1}!  ({0} 异常停止,{1}！)", disc, str_status);
                                if (bshowmsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR,ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                                Stop();
                                return res = EM_RES.MOVE_ERR;
                            }
                            break;
                        }
                    }
                ...
                {
                    Stop();
                    ErrMes = VAR.IsChinese?string.Format("{0} 等待停止超时({1}ms)！", disc, timeout) : string.Format("{0} Wait to stop timeout ({1}ms)！ ({0} 等待停止超时({1}ms)！)",  disc, timeout);
                    if (bshowmsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.Timeout, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                    return res=EM_RES.MOVE_TIMEOUT;
                }
                if (timeout_ms > 0)
                {
                    timeout_ms-=5;
        ...
        public EM_RES WaitINP(ref bool pbquit, int timeout_ms=100, bool bdoevent = false)
        {
            ...
            while (true)
            {
                ...
                {
                    //Stop();
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 等待到位信号超时({1}ms)！", disc, time_temp): string.Format("{0}  INP timeout({1}ms)！  ({0} 等待到位信号超时({1}ms)！)",  disc, time_temp), DReport.EmErrCode.Timeout, (int)DReport.EmHareware.Axis + m_id);
                    return res = EM_RES.TIMEOUT;
                }
                if (timeout_ms > 0)
                {
        ...
        public EM_RES PVTMove(double tri_spd, double mov_spd, double[] tri_pos, double stop_pos)
        {
            ...
            //debug
            string str = VAR.IsChinese?string.Format("PVT:{0},飞拍速度{1},定位速度{2}", disc, tri_spd, mov_spd): string.Format("PVT:{0},tri_spd:{1},mov_spd:{2}   (PVT:{0},飞拍速度{1},定位速度{2})", disc, tri_spd, mov_spd);
            ...
            if (res != EM_RES.OK)
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 设置PVT表格出错，{1}", disc, res): string.Format("{0} ERROE:Set PVT table,{1}   ({0} 设置PVT表格出错，{1})", disc, res), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                return res;
            }
            ...
            if (res != EM_RES.OK)
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} Pvt运行出错,{1}",disc, res): string.Format("{0} ERROR:Pvt Move,{1}  ({0} Pvt运行出错,{1})", disc, res), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                return res;
            }
        ...
        TRYAGAIN:
            ...
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 未回零!", disc): string.Format("{0}  Please reset!  ({0} 未回零!)", disc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.WAR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id,ERR_ALM.EmErrItem.MoveError);
                return res = EM_RES.MOVE_ERR;
            }
            ...
            if (pos > slp || pos < sln)
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 目标位 {1:F3} 超软限位范围 [{2:F3},{3:F3}]", disc, pos, sln, slp): string.Format("{0} pos:{1:F3} over range [{2:F3},{3:F3}]   ({0} 目标位 {1:F3} 超软限位范围 [{2:F3},{3:F3}])",  disc, pos, sln, slp);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id,ERR_ALM.EmErrItem.LimitProtect);
                return res = EM_RES.MOVE_PARA_ERR;
            }
            ...
            switch (status)
            {
                case AX_STA.EMG:
                case AX_STA.DIS:
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, disc + str_status , DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id,ERR_ALM.EmErrItem.StautsAbnomal);
                    ret = EM_RES.MOVE_ERR;
                    break;
                case AX_STA.ALM:
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, disc + str_status , DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id,ERR_ALM.EmErrItem.StautsAbnomal);
                ...
                case AX_STA.PTP:
                    ...
                    //else
                    {
                        ...
                        if (ret != EM_RES.OK)
                        {
                            ErrMes = VAR.IsChinese?String.Format("{0} 等待定位完成超时(5S)!", disc): String.Format("{0} Wait for stop timeout(5s)!  ({0} 等待定位完成超时(5S)!)", disc);
                            VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.Timeout, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.TimeOut);
                            ret = EM_RES.MOVE_ERR;
                            goto TRYAGAIN;
                        }
                    }
                ...
                case AX_STA.READY:
                    ...
                    {
                        ErrMes = VAR.IsChinese?String.Format("{0} 异常, {1}", disc, str_status): String.Format("{0} Error ,{1}  ({0} 异常, {1})", disc, str_status);
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.LimitProtect);
                        ret = EM_RES.MOVE_ERR;
                        break;
                    }
                    ...
                    if (card.brand == CARD.BRAND.LEADSHINE)
                    {
                        ...
                        if (err != 0)
                        {
                            ErrMes = VAR.IsChinese?String.Format("{0} 定位命令出错，0x{1:X8}", disc, err): String.Format("{0} ERROR:Move,0x{1:X8}  ({0} 定位命令出错，0x{1:X8})", disc, err);
                            VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                            ret = EM_RES.MOVE_ERR;
                        }
                    }
#endif
        ...
        public EM_RES JOG_Move(ref bool bquit,AX_DIR dir, double spd, bool bstep = false, double step=0)
        {
            ...
            //if (card.brand != CARD.BRAND.ORIENTALMOTOR)
            {
                ...
                {
                    Stop();
                    ErrMes = VAR.IsChinese?string.Format("{0} 点动，软限位!", disc): string.Format("{0}  Jog move,software limit!  ({0} 点动，软限位!)", disc);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SoftwareProtect, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.LimitProtect);
                    return res = EM_RES.ERR;
                }
            }
            ...
            if (ChkSafeSen != null)
            {
                ...
                {
                    Stop();
                    ErrMes = VAR.IsChinese?string.Format("{0} 点动, 安全防护 ，停止!", disc): string.Format("{0} Jog move,safe protect,Stop!  ({0} 点动, 安全防护 ，停止!)", disc);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.HarewareProtect, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveProtect);
                    return res;
                }
            }
            ...
            if ((card.brand != CARD.BRAND.ORIENTALMOTOR) && status != AX_STA.READY)
            {
                ...
                {
                    Stop();
                    ErrMes = VAR.IsChinese?string.Format("{0} 点动出错,{1}", disc, str_status): string.Format("{0} ERROR:Jog move,{1}  ({0} 点动出错,{1})", disc, str_status);                   
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                    return res = EM_RES.OK;
                }
            }            
            ...
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 点动设置速度出错!", disc): string.Format("{0}ERROR:Set Jog spd!    ({0} 点动设置速度出错!)", disc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.ParmError);
                return EM_RES.ERR;
            }
            ...
            {
                ErrMes = VAR.IsChinese?string.Format("{0} 点动速度异常!", disc): string.Format("{0}ERROR:Jog spd!  ({0} 点动速度异常!)", disc);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                return EM_RES.ERR;
            }
            ...
            if (bstep == false)
            {
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese?string.Format("{0} JOG定速命令错误,Err:{1}！", disc, err): string.Format("{0}  ERROR:Jog single move,{1}!  ({0} JOG定速命令错误,Err:{1}！)", disc, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                    Stop();
                    return res = EM_RES.ERR;
                }
            ...
            else
            {
                ...
                if (dir == AX_DIR.P && (fcmd_pos + step) > slp || dir == AX_DIR.N && (fcmd_pos - step) < sln)
                {
                    ...
                    {
                        Stop();
                        ErrMes = VAR.IsChinese?string.Format("{0} 点动，软限位!", disc): string.Format("{0}  Jog move,Soft limit  ({0} 点动，软限位!)",  disc);
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SoftwareProtect, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.LimitProtect);
                        return res = EM_RES.ERR;
                    }
                }
                ...
                {
                    res = EM_RES.ERR;
                    ErrMes = VAR.IsChinese ? string.Format("{0} JOG点动命令出错, Err:{1}", disc, err): string.Format("{0}  JOG move,ERROR:{1}  ({0} JOG点动命令出错, Err:{1})", disc, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id, ERR_ALM.EmErrItem.MoveError);
                    Stop();
                }
        ...
        public EM_RES HomeOffset(ref bool bquit, int timeout_ms = 100, double offset = double.MaxValue, bool bdoevent = true)
        {
            ...
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("{0} 偏移={1}，超出软限位范围！", disc, offset): string.Format("{0}  offset={1},exceeding the soft limit ({0} 偏移={1}，超出软限位范围！)", disc, offset), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Axis + m_id);
                home_status = HOME_STA.ERROR;
                return EM_RES.PARA_ERR;
            }
            ...
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ex.Message, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                home_status = HOME_STA.ERROR;
                return res = EM_RES.ERR;
            }
        ...
        public EM_RES Home(ref bool bquit,double spd, ushort mode, AX_DIR dir, int timeout_ms = 0, bool bdoevent = true)
        {
            ...
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?"急停信号按下":"Pressed the EMG",DReport.EmErrCode.EmgStop,(int)DReport.EmHareware.Axis + m_id);
                Stop();
                return res = EM_RES.ERR;
            }
            if (status == AX_STA.ALM && card.brand != CARD.BRAND.ORIENTALMOTOR)
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR,  disc +str_status, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                Stop();
                return res = EM_RES.ERR;
            }
            ...
            if (card.brand == CARD.BRAND.LEADSHINE)
            {
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese?string.Format("{0}设置回零模式出错,0x{1:X8}", disc, err): string.Format("ERROR:{0} Set HOME MODE,0x{1:X8}  ({0}设置回零模式出错,0x{1:X8})",  disc, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Axis + m_id);
                    return res = EM_RES.ERR;
                }
                ...
                if (err != 0)
                {
                    ErrMes = VAR.IsChinese ? string.Format("{0}回零出错,0x{1:X8}", disc, err): string.Format(" {0} ERROR:home,0x{1:X8}   ({0}回零出错,0x{1:X8})",  disc, err);
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, ErrMes, DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Axis + m_id);
                    home_status = HOME_STA.ERROR;
                    return res = EM_RES.ERR;
                }
            }
        ...
        void task()
        {
            ...
            try
            {
                ...
                if (card.brand == CARD.BRAND.LEADSHINE)
                {
                    ...
                    {
                        home_task_ret = Home(ref bhomequit, home_spd / 3, 1, home_dir, 10000);
                        Stop();
                        if (home_task_ret != EM_RES.OK) return;
                    }
                    ...
                    {
                        home_task_ret = Home(ref bhomequit, home_spd, 0, home_dir == AX_DIR.N ? AX_DIR.P : AX_DIR.N, 40000);
                        Stop();
                        if (home_task_ret != EM_RES.OK) return;
                    }
                ...
                spd_start = spd_temp;
                home_task_ret = Home(ref bhomequit, home_spd, home_mode, home_dir, 40000);
                Stop();
                if (home_task_ret != EM_RES.OK) return;

                //再次回零
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.DBG, string.Format("{0} rehome...",  disc ));
                home_task_ret = Home(ref bhomequit, home_spd, home_mode, home_dir, 10000);
                Stop();
                if (home_task_ret != EM_RES.OK) return;
            ...
            finally
            {
                ...
                spd_start = spd_temp;
                //result
                EM_RES res1 = EnSlm(true);
                if (res1 != EM_RES.OK) home_task_ret = EM_RES.ERR;
