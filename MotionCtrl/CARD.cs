...
namespace MotionCtrl
{
    ...
    public class CARD
    {
        ...
        public EM_RES UpdateIOBuff(bool bforceupdate = true)
        {
            ...
            {
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, string.Format("{0} update IO ,Err:{1},Cnt:{2},T{3}ms", disc, ret, list_temp.Count, CardIOBuf.tickcnt - t), DReport.EmErrCode.UpdateError, (int)DReport.EmHareware.Card+ card_id);
                return EM_RES.ERR;
            }
#endif
            return EM_RES.OK;
        ...
        #endregion
        #region 构造
        public CARD(int id, int card_id, int ax_num, int input_num, int output_num, BRAND brand, TYPE type, string disc, string ps = "",string englishps="")
        {
            this.m_id = id;
            ...
            this.type = type;
            this.ps = ps;
            this.englishps = englishps;
            isInit = false;
            handle = (IntPtr)0;
            str_disc = disc;
        }
        public CARD(int id, string ip, int ax_num, int input_num, int output_num, BRAND brand, TYPE type, string disc, string ps = "", string englishps = "")
        {
            this.m_id = id;
            this.ip = ip;
            this.ax_num = ax_num;
        ...
        public CARD(int id, string port_name, int baudrate, BRAND brand, TYPE type, string disc, string ps = "", string englishps = "")
        {
            this.m_id = id;
            this.ax_num = 10000;
        ...
        public EM_RES Init(int card_id = -1, int maincard_id = -1, string ip = "", string filename = "")
        {
            ...
            if (brand == BRAND.LEADSHINE)
            {
                if (type == TYPE.MOTION)
                {
                    ...
                    if (ret < 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0}ID有重复，请检查确认!0x{1:X8}", disc, ret): string.Format("{0}ID has duplicates, please check to confirm!0x{1:X8}   ({0}ID有重复，请检查确认!0x{1:X8})", disc, ret), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    ...
                    if (ret != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, string.Format("ERROR:Get{0} list,0x{1:X8}   (获取{0}列表错误，0x{1:X8})", disc, ret), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    ...
                    {
                        isInit = false;
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0}列表找不到ID为{1}的板卡!", disc, card_id): string.Format("Can't find the card from {0} list where ID is {1}    ({0}列表找不到ID为{1}的板卡!)", disc, card_id), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    ...
                    isInit = true;
                    handle = (IntPtr)1;
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0}初始化成功!", disc): string.Format("{0}Init successfully!   ({0}初始化成功!)", disc));
                    return EM_RES.OK;
                }
                else if (type == TYPE.CAN_IO)
                {
                    ...
                    if (ret != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("{0}连接失败,Err:0x{1:X8},重新连接...", disc, ret): string.Format("{0}Link failed,Err:0x{1:X8},reconnect...    ({0}连接失败,Err:0x{1:X8},重新连接...)", disc, ret), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                        ret = LTDMC.dmc_set_can_state((ushort)maincard_id, (ushort)card_id, 1, 0);
                        if (ret != 0)
                        {
                            VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("{0}重新连接失败,请确认卡连接是否正确!Err:0x{1:X8}", disc, ret): string.Format("{0}Reconnection failed, please confirm that the card is connected correctly!Err:0x{1:X8}   ({0}重新连接失败,请确认卡连接是否正确!Err:0x{1:X8})", disc, ret), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                            return EM_RES.ERR;
                        }
                    }
                    isInit = true;
            ...
            if (brand == BRAND.ZMOTION)
            {
                ...
                if (ip.Length < 7)
                {
                    isInit = false;
                    handle = (IntPtr)0;
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0} IP异常,{1}", disc, ip): string.Format("{0} ERROR:IP,{1}   ({0} IP异常,{1})", disc, ip), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                    return EM_RES.CAM_PARA_ERR;
                }

                VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0} 连接...", disc): string.Format("{0} linking...   ({0} 连接...)", disc));
                ...
                if (ret == 0)
                {
                    ...
                    if (ret != 0)
                    {
                        isInit = false;
                        handle = (IntPtr)0;
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0}初始化失败,IP={1},Err:{2}", disc, ip, ret): string.Format("{0} initialize failed,IP={1},Err:{2}    ({0}初始化失败,IP={1},Err:{2})", disc, ip, ret), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    else
                    {
                ...
                else
                {
                    isInit = false;
                    handle = (IntPtr)0;
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0}找不到，IP={1}", disc, ip): string.Format("Can't find {0},IP={1}   ({0}找不到，IP={1})", disc, ip), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                    return EM_RES.ERR;
                }
            }
#endif
            ...
            if (brand == BRAND.ORIENTALMOTOR)
            {
                ...
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("InitRs485异常，BRAND={0}，TYPE={1},Port={2}/{3}", brand, type, PortName, Baudrate): string.Format("ERROR: InitRs485,BRAND={0}，TYPE={1},Port={2}/{3}   (InitRs485异常，BRAND={0}，TYPE={1},Port={2}/{3})", brand, type, PortName, Baudrate), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
                    return EM_RES.ERR;
                }
            }
#endif
            VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese ? string.Format("未定义异常，BRAND={0}，TYPE={1}", brand, type): string.Format("ERROR:Undefined,BRAND={0}，TYPE={1}   (未定义异常，BRAND={0}，TYPE={1})", brand, type), DReport.EmErrCode.ConnectFailed, (int)DReport.EmHareware.Card + card_id);
            return EM_RES.ERR;
        }
        /// <summary>
        /// 初始化板卡。
        ...
        public EM_RES DownLoadFile(string filename = "")
        {
            ...
            {
                //download config
                if (filename == "") filename = string.Format("{0}DMC3C00_{1}.ini", Path.GetFullPath("..") + "\\syscfg\\", card_id);
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?String.Format("{0}下载配置,{1}", str_disc, filename): String.Format("{0}Download cfg,{1}   ({0}下载配置,{1})", str_disc, filename));
                ret = LTDMC.dmc_download_configfile((ushort)card_id, filename);
                if (ret != 0)
                {
                    LTDMC.dmc_board_close();
                    isInit = false;
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?String.Format("{0}/id{1},配置失败！Err:0x{2:X8}", disc, card_id, ret): String.Format("{0}/id{1},Init failed!Err:0x{2:X8}   ({0}/id{1},配置失败！Err:0x{2:X8})", disc, card_id, ret), DReport.EmErrCode.GetParamError, (int)DReport.EmHareware.Card + card_id);
                    return EM_RES.ERR;
                }
                else VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?String.Format("{0}/id{1},更新配置成功！", disc, card_id): String.Format("{0}/id{1},update configuration successfully!   ({0}/id{1},更新配置成功！)", disc, card_id));
            }
#endif
#if ZMOTION
            if (brand == BRAND.ZMOTION)
            ...
            if (brand == BRAND.ORIENTALMOTOR)
            {
                ...
                VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?String.Format("{0}下载配置", str_disc): String.Format("{0} Download cfg   ({0}下载配置)", str_disc));
                foreach (AXIS ax in AxList)
                {
                    UInt32 temp = 0;
                    UInt32 temp2 = 0;
        ...
        public EM_RES ChkOnline(string filename = "")
        {
            ...
            if (brand == BRAND.ZMOTION)
            {
                ...
                if (((long)handle) == 0)
                {
                    if (bshowmsg) VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 掉线!", disc): string.Format("{0}Disconnect!    ({0} 掉线!)", disc), DReport.EmErrCode.ComOffline, (int)DReport.EmHareware.Card + card_id);
                    bshowmsg = false;
        ...
        public EM_RES Close()
        {
            ...
            if (brand == BRAND.LEADSHINE)
            {
                if (type == TYPE.MOTION)
                {
                    ...
                    if (ret != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0}关闭失败,Err:0x{1:X8}", disc, ret): string.Format("Close failed,Err:0x{1:X8}   ({0}关闭失败,Err:0x{1:X8})", disc, ret), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0}关闭成功", disc): string.Format("Close successfully   ({0}关闭成功)", disc));
                    isInit = false;
                    return EM_RES.OK;
                }
                else if (type == TYPE.CAN_IO)
                {
                    ...
                    if (ret != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0}断开失败,Err:0x{1:X8}", disc, ret): string.Format("{0}Disconnect failed,Err:0x{1:X8}   ({0}断开失败,Err:0x{1:X8})", disc, ret), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese ? string.Format("{0}断开成功", disc): string.Format("{0}Disconnect successfully  ({0}断开成功)", disc));
                    isInit = false;
                    return EM_RES.OK;
                }
#if LEADSHINE_IO
            ...
            if (brand == BRAND.ZMOTION)
            {
                if (handle != (IntPtr)0)
                {
                    ...
                    if (ret != 0)
                    {
                        VAR.msg.AddMsg(Msg.EM_MSGTYPE.SYS, VAR.IsChinese?string.Format("{0}关闭失败,Err:0x{1:X8}", disc, ret): string.Format("{0}Close failed,Err:0x{1:X8}   ({0}关闭失败,Err:0x{1:X8})", disc, ret), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Card + card_id);
                        return EM_RES.ERR;
                    }
                    handle = (IntPtr)0;
                    isInit = false;
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.DBG, VAR.IsChinese?string.Format("{0}关闭成功", disc): string.Format("{0}Close successfully   ({0}关闭成功)", disc));
                    return EM_RES.OK;
                }
                return EM_RES.OK;
            }
...
#endif
            VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("未定义异常，BRAND={0}，TYPE={1}", brand, type): string.Format("ERROR:Undefined,BRAND={0}，TYPE={1}   (未定义异常，BRAND={0}，TYPE={1})", brand, type), DReport.EmErrCode.SetParamError, (int)DReport.EmHareware.Card + card_id);
            return EM_RES.ERR;
        }
        #endregion
        #region 设置轴到工作速度
        ...
        public EM_RES AllCardStop(int mode = 2)
        {
            ...
            if (brand == BRAND.ZMOTION)
            {
                ...
                if (ret != 0)
                {
                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.ERR, VAR.IsChinese?string.Format("{0} 停止出错，ERR:{1}", disc, ret): string.Format("{0}ERROR:Stop,{1}   ({0} 停止出错，ERR:{1})", disc, ret), DReport.EmErrCode.MoveError, (int)DReport.EmHareware.Card + card_id);
                    return EM_RES.ERR;
                }
            }
#endif
