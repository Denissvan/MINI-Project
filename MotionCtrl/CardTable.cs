...
namespace MotionCtrl
{
    public partial class CardTable : UserControl
    {
        ...
        private  void FillTableWithAxisInf(CARD card, int row=-2)
        {
            ...
            {
                dgv.Rows[row].Cells[0].Value = card.disc;
                dgv.Rows[row].Cells[1].Value = card.isReady ? "Ready" : "Error";
                dgv.Rows[row].Cells[2].Value = card.id;
                dgv.Rows[row].Cells[3].Value = card.ip.Length > 0 ? card.ip : (card.maincard_id > 0 ? string.Format("{0}/{1}", card.card_id, card.maincard_id) : card.card_id.ToString());
                dgv.Rows[row].Cells[4].Value = card.ax_num;
                dgv.Rows[row].Cells[5].Value = card.output_num;
                dgv.Rows[row].Cells[6].Value = card.input_num;
                dgv.Rows[row].Cells[7].Value = card.brand;
                dgv.Rows[row].Cells[8].Value = card.englishps;
            }
        }
        ...
        private void dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show(VAR.IsChinese?"卡列表异常，请重新启动软件!": "Card list is abnormal, please restart the software!          \r\n(卡列表异常，请重新启动软件!)");
                return;
            }
            ...
            if (e.ColumnIndex == 9)
            {
                ...
                if (ret != EM_RES.OK) MessageBox.Show(VAR.IsChinese?list_card.ElementAt(e.RowIndex).disc + "打开异常!Err:" + ret.ToString(): list_card.ElementAt(e.RowIndex).disc + "Init Err!Err:" + ret.ToString()+"\r\n"+ list_card.ElementAt(e.RowIndex).disc + "打开异常!Err:" + ret.ToString());
            }
            //关闭
            else if (e.ColumnIndex == 10)
            {
                ret = list_card.ElementAt(e.RowIndex).Close();
                if (ret != EM_RES.OK) MessageBox.Show(VAR.IsChinese ? list_card.ElementAt(e.RowIndex).disc + "关闭异常!Err:" + ret.ToString() : list_card.ElementAt(e.RowIndex).disc + "Close Err!Err:" + ret.ToString() + "\r\n" + list_card.ElementAt(e.RowIndex).disc + "关闭异常!Err:" + ret.ToString());
            }
        }
        ...
        public void ChangeColumn()
        {
            if (VAR.IsChinese)
            {
                ...
                id.HeaderText = "ID";
                ip.HeaderText = "IP/ID";
                ax_num.HeaderText = "轴数";
                out_num.HeaderText = "输出";
                in_num.HeaderText = "输入";
                brand.HeaderText = "品牌";
                ps.HeaderText = "备注";
                btn_close.Text = "关闭";
                btn_open.Text = "打开";
            ...
            else
            {
                ...
                id.HeaderText = "ID";
                ip.HeaderText = "IP/ID";
                ax_num.HeaderText = "Num";
                out_num.HeaderText = "Out";
                in_num.HeaderText = "In";
