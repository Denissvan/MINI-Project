...
namespace SunyLib.TCPHelper
{
    public partial class AxTcpServer : Component
    {
                ...
                while (IsStartListening)
                {
                    ...
                    try
                    {
                       ...
                       OnTcpServerReturnClientCountEnterHead(ClientSocketList.Count);
                        ThreadPool.QueueUserWorkItem(ClientSocketCallBack, clientSocket);
                    }
                    catch (Exception)
                    {
        ...
        public void ClientSocketCallBack(Object obj)
        {
            ...
            while (IsStartListening)
            {
                ...
                try
                {
                    ...
                    var bytelen = temp.Receive(receivebyte);
                    if (bytelen > 0)
                    {
                        //接收客户端数据
        ...
        {
            OnReceviceByte?.Invoke(temp, dataBytes);
        } 
        #endregion
        ...
        {
            OnReturnClientCount?.Invoke(count);
        } 
        #endregion
