...
namespace UI
{
    public partial class traybox : UserControl
    {
            ...
            set
            {
                ...
                switch(value)
                {
                    case TrayBox.EM_DIR.ONLY_IN:
                        btn_tray_in.Visible = true;
                        btn_tray_out.Visible = false;
                        break;
                    case TrayBox.EM_DIR.ONLY_OUT:
                        btn_tray_in.Visible = false;
                        btn_tray_out.Visible = true;
                        break;
        ...
        public void UpdateShow()
        {
            ...
            if (btsk == false)
            {
                Task tsk = new Task(() =>
                {
                    ...
                    {
                        btsk = true;
                        if (box.out_tray_hd != null) box.out_tray_hd._isON = box.out_tray_hd.isON;
                        if (box.in_tray_hd != null) box.in_tray_hd._isON = box.in_tray_hd.isON;
                        if (box.in_box_sen != null) box.in_box_sen._isON = box.in_box_sen.isON;
                    }
                    finally
                    {
                        btsk = false;
            ...
            chk_zk_on.Checked = box.out_tray_hd != null ? box.out_tray_hd._isON : false;
            chk_zk_sen.Checked = box.in_tray_hd != null ? box.in_tray_hd._isON : false;
            chk_box_sen.Checked = box.in_box_sen != null ? box.in_box_sen._isON : false;
        ...
        private void btn_up_Click(object sender, EventArgs e)
        {
            ...
            box.Up(ref VAR.gsys_set.bquit,false);
            pnl_status.Refresh();
        }
        ...
        private void btn_down_Click(object sender, EventArgs e)
        {
            if (VAR.gsys_set.status == EM_SYS_STA.RUN) return;
            VAR.gsys_set.bquit = false;
            box.Down(ref VAR.gsys_set.bquit,false);
            pnl_status.Refresh();
        }
        ...
        private void btn_in_Click(object sender, EventArgs e)
        {
            if (VAR.gsys_set.status == EM_SYS_STA.RUN) return;
            VAR.gsys_set.bquit = false;
            box.In(ref VAR.gsys_set.bquit,false);
            pnl_status.Refresh();
        }
        ...
        private void btn_out_Click(object sender, EventArgs e)
        {
            if (VAR.gsys_set.status == EM_SYS_STA.RUN) return;
            VAR.gsys_set.bquit = false;
            box.Out(ref VAR.gsys_set.bquit,false);
            pnl_status.Refresh();
        }
    }
}
