...
namespace UI
{
    public partial class FrMain : Form
    {
        ...
        private void FrMain_Load(object sender, EventArgs e)
        {
            ...
            lbVer.Text = string.Format("WLTmini2   V{0}.{1}", Assembly.GetExecutingAssembly().GetName().Version.Major, Assembly.GetExecutingAssembly().GetName().Version.Minor);
            //load sys config
        ...
        private void timer_key_Tick(object sender, EventArgs e)
        {
            ...
            //if (lb_Date.Bounds.Contains(Control.MousePosition.X-this.Left, Control.MousePosition.Y-this.Top))
            //    lbl_update.Text = "是";
            //else
            //    lbl_update.Text = "否x:"+ Control.MousePosition.X.ToString("f0")+"y:"+ Control.MousePosition.Y.ToString("f0");


            //    if (MT.isReady == false) return;
            //    #region JOG stop when no key down
            //    if (VAR.gsys_set.status != CONST.SYS_STATUS_RUN && bJOG)
            //    {
            //        MT.AXIS_X.JOG_ChkAndStop();
            //        MT.AXIS_Y.JOG_ChkAndStop();
            //        if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.Up) & System.Windows.Input.KeyStates.None) > 0)
            //            if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.Down) & System.Windows.Input.KeyStates.None) > 0)
            //                if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.PageUp) & System.Windows.Input.KeyStates.None) > 0)
            //                    if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.PageDown) & System.Windows.Input.KeyStates.None) > 0)
            //                        if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.Left) & System.Windows.Input.KeyStates.None) > 0)
            //                            if ((System.Windows.Input.Keyboard.GetKeyStates(System.Windows.Input.Key.Right) & System.Windows.Input.KeyStates.None) > 0)
            //                            {
            //                                MT.AXIS_X.JOG_Stop();
            //                                MT.AXIS_Y.JOG_Stop();
            //                                VAR.msg.AddMsg(Msg.EM_MSGTYPE.NOR, "JOG stop");
            //                                bJOG = false;
            //                            }
            //    }
            //    #endregion

            //    if (VAR.gsys_set.status == CONST.SYS_STATUS_PAUSE) COUNT_DATA.ct_pause += timer_key.Interval;

            //    //在待机或暂停状态，安全光栅触发时，停止手动移动。
            //    if ((VAR.gsys_set.status == CONST.SYS_STATUS_STANDBY || VAR.gsys_set.status == CONST.SYS_STATUS_PAUSE) && MT.ChkSafeSen()!=CONST.RES_OK)
            //    {
            //        MT.AllAxStopExceptFd();
            //    }

            //    if (VAR.gsys_set.status == CONST.SYS_STATUS_EMG)
            //    {
            //        if (MT.GPIO_IN_EMG.isOFF)
            //            Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, "急停", -1);
            //        else
            //        {
            //            Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_YELLOW_FLASH, string.Format("待复位"), -1, true);
            //            VAR.gsys_set.status = CONST.SYS_STATUS_UNKOWN;
            //        }
            //        return;
            //    }
            ...
            //            if (key_emg_cnt > 1)
            //            {
            //                Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, "急停");
            //                VAR.msg.AddMsg(Msg.EM_MSGTYPE.NOR, "急停按键按下");
            //                VAR.gsys_set.status = CONST.SYS_STATUS_EMG;
            ...
            //    if (!MT.AXIS_X.isINP && MT.AXIS_X.isALM)
            //    {
            //        Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, "伺服失电", -1);
            //        VAR.gsys_set.status = CONST.SYS_STATUS_ERR;
            //        VAR.gsys_set.bquit = true;
            //        return;
            //    }
            ...
            //                {
            //                    Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, "急停");
            //                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.NOR, ax.disc + " 急停");
            //                }
            //                Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, "急停", -1);
            //                VAR.gsys_set.status = CONST.SYS_STATUS_EMG;
            //                VAR.gsys_set.bquit = true;
            ...
            //                if (VAR.gsys_set.status != CONST.SYS_STATUS_ERR)
            //                {
            //                    Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, string.Format("{0}/{1}轴报警", ax.card_id, ax.num));
            //                    VAR.msg.AddMsg(Msg.EM_MSGTYPE.NOR, ax.disc + " 报警");
            //                }
            //                Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_RED_FLASH, string.Format("{0}/{1}轴报警", ax.card_id, ax.num), -1);
            //                VAR.gsys_set.status = CONST.SYS_STATUS_ERR;
            //                VAR.gsys_set.bquit = true;
            //                balm = true;
            //                break;
            //            }
            //        }
            //        if (!balm && (VAR.inf_string.Contains("急停") || VAR.inf_string.Contains("轴报警")))
            //        {
            //            Action.VAR.sys_inf.Set(CONST.EM_ALM_STA.WAR_YELLOW_FLASH, string.Format("待复位"), -1, true);
            //            VAR.gsys_set.status = CONST.SYS_STATUS_UNKOWN;
            //        }
            ...
            //    //系统异常返回
            //    if (VAR.gsys_set.status == CONST.SYS_STATUS_ERR || VAR.gsys_set.status == CONST.SYS_STATUS_WARNING || VAR.gsys_set.status == CONST.SYS_STATUS_EMG)
            //    {
            //        VAR.gsys_set.bquit = true;
            //    }
            ...
            //                //chk alm
            //                foreach (AXIS ax in MT.AxisList)
            //                {
            //                    if (ax.isEMG || ax.isALM) return;
            //                }

            //                //chk safe
            //                if (MT.ChkSafeSen()!=CONST.RES_OK) return;

            //                //run
            //                VAR.gsys_set.user_idx = 0;
            //                VAR.gsys_set.bquit = false;
            //                Action.th_run();
            //            }
            //        }
