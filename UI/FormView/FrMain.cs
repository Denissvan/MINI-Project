...
namespace UI
{
    public partial class FrMain : Form
    {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            warn.ok_txt = MultiLanguage.TxtSelct("确定", "OK", "V?NG");
            warn.ws = null;
            warn.title = MultiLanguage.TxtSelct("提示:设备维修!", "Tip: Equipment maintenance!", "M?o: B?o trì thi?t b?!");
            warn.msg = MultiLanguage.TxtSelct("当前设备在维修状态", "The current equipment is in maintenance status!", "Thi?t b? hi?n t?i ?ang trong tình tr?ng b?o trì!");
            ...
            warn.lb_msg = MultiLanguage.TxtSelct(
                ...
                "M?o: Thi?t b? hi?n t?i ?ang ???c s?a ch?a, vui lòng xác nh?n r?ng kh?ng có nhan viên b?o trì trong thi?t b? tr??c khi ?óng h?p tho?i này! ");
            MT.Display_frwarn(fr_warn, warn, ERR_ALM.EmErrItem.Null, false);
            VAR.gsys_set.status = sys_status;
            //MT.ST_WARN warn = new MT.ST_WARN();
            //warning fr_warn = new warning();//增加语言
            //warn.ok_txt = MultiLanguage.TxtSelct("继续", "GoOn", "?i ti?p");
            //warn.cancle_txt = MultiLanguage.TxtSelct("停止", "Stop", "Ng?ng l?i");
            //warn.title = MultiLanguage.TxtSelct("提示:测试项匹配异常", "Tip: The test item does not match properly", "M?u th? kh?p v?i d? th??ng");
            //VAR.sys_inf.Set(EM_ALM_STA.WAR_YELLOW_FLASH, VAR.IsChinese ? "测试项匹配异常" : " match err", 10, true);
            //warn.msg = MultiLanguage.TxtSelct("测试项匹配异常!", " match err", "M?u th? kh?p v?i d? th??ng");
